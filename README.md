...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .staff-avatar.staff-photo{width:48px;height:48px;object-fit:cover;object-position:center;padding:0;border-radius:4px;display:block;font-size:0;}
  ...
  /* ===== VOTE SECTION ===== */
  .vote-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;margin-bottom:0;}
  .vote-card{background:rgba(0,0,0,.3);border:1px solid rgba(79,228,228,.18);border-radius:3px;padding:20px;text-align:center;transition:all .25s;}
  .vote-card:hover{border-color:rgba(79,228,228,.35);transform:translateY(-4px);}
  .vote-icon{font-size:32px;margin-bottom:12px;display:block;}
  .vote-title{font-family:'Press Start 2P',monospace;font-size:8px;color:var(--diamond);letter-spacing:2px;margin-bottom:16px;text-transform:uppercase;}
  .vote-btn{width:100%;padding:12px;background:linear-gradient(180deg,rgba(79,228,228,.3),rgba(79,228,228,.1));border:1px solid rgba(79,228,228,.4);color:var(--diamond);font-family:'Press Start 2P',monospace;font-size:8px;letter-spacing:2px;border-radius:2px;cursor:pointer;transition:all .2s;}
  .vote-btn:hover:not(:disabled){background:linear-gradient(180deg,rgba(79,228,228,.5),rgba(79,228,228,.25));border-color:var(--diamond);transform:scale(1.02);}
  .vote-btn:disabled{opacity:.5;cursor:not-allowed;}
  .vote-cooldown{font-family:'VT323',monospace;font-size:13px;color:rgba(255,100,100,.7);margin-top:8px;letter-spacing:1px;display:none;}
...
<nav class="nav">
  <div class="nav-inner">
    ...
    <div class="nav-links" id="navLinks">
      ...
      <button class="nav-link" data-section="vote" onclick="showSection('vote',9)">🗳️ Vote</button>
    </div>
  </div>
</nav>
...
<div class="content">
        ...
        <div class="event-card featured">
          ...
          <div class="event-content">
            <div class="event-title-wrap">
              ...
              <div class="event-date">📅 26 Mei 2026 — Malam Hari</div>
            </div>
            ...
            <p class="event-rules">
              <strong>📌 Cara Ikutan:</strong> Datang ke plaza server pada waktu yang ditentukan dan ikuti mini-games menarik. Pemenang akan mendapatkan senjata brutal legend yang keren!
            </p>
          </div>
        </div>
    ...
    <div class="mc-card">
      ...
      <div class="staff-grid">
        ...
        <div class="staff-card owner">
          ...
          <img src="https://i.imgur.com/MRFdSyS.jpeg" alt="Fadlangemer" class="staff-avatar owner-av staff-photo"/>
        ...
        <div class="staff-card admin">
          <img src="https://i.imgur.com/ZDliPKp.png" alt="Lalo" class="staff-avatar admin-av staff-photo"/>
          <div class="staff-info">
  ...
  <div class="section" id="section-vote">
    <div class="header">
      <div class="server-badge">🗳️ DUKUNG SERVER</div>
      <h2 class="server-title" style="font-size:clamp(14px,4vw,28px);">VOTE<br><span class="title-accent">SERVER</span></h2>
      <p class="subtitle">▸ BANTU VALORIX NAIK DI RANKING ◂</p>
    </div>

    <div class="mc-card">
      <div class="cc-tr"></div><div class="cc-bl"></div>
      <p style="font-family:'VT323',monospace;font-size:16px;color:rgba(200,230,255,.65);letter-spacing:1px;margin-bottom:20px;line-height:1.6;">
        Voting membantu server Valorix Nation naik ranking di situs server lists. Setiap 24 jam kamu bisa vote 1x per site. Terima kasih atas dukunganmu! 🙏
      </p>

      <div class="vote-grid">
        <div class="vote-card">
          <div class="vote-icon">⭐</div>
          <div class="vote-title">TopG.org</div>
          <button id="vote-btn-topg" class="vote-btn" onclick="openVoteLink('topg')">
            🗳️ VOTE DI TOPG
          </button>
          <div id="vote-cooldown-topg" class="vote-cooldown"></div>
        </div>

        <div class="vote-card">
          <div class="vote-icon">🎮</div>
          <div class="vote-title">MinecraftServers.org</div>
          <button id="vote-btn-minecraft_servers" class="vote-btn" onclick="openVoteLink('minecraft_servers')">
            🗳️ VOTE DI MCSERVERS
          </button>
          <div id="vote-cooldown-minecraft_servers" class="vote-cooldown"></div>
        </div>
      </div>

      <p style="font-family:'VT323',monospace;font-size:14px;color:rgba(200,230,255,.45);text-align:center;margin-top:16px;letter-spacing:1px;">
        💡 Tip: Setiap vote membuat server kami lebih terkenal!
      </p>
    </div>
  </div>
...
<script>
// ===== NAV ORDER =====
const sectionOrder = ['home','features','gallery','rank','panduan','event','social','staff','lapor','vote'];
...
// ===== VOTE SYSTEM =====
const VOTE_CONFIG = {
  topg: { name: 'TopG.org', url: 'https://topg.org/minecraft-servers/server-679293' },
  minecraft_servers: { name: 'MinecraftServers.org', url: 'https://minecraftservers.org/server/686562' }
};
const VOTE_STORAGE_KEY = 'valorix_vote_data';
const VOTE_COOLDOWN_HOURS = 24;

function openVoteLink(voteType){
  if(!VOTE_CONFIG[voteType]) return;
  const config = VOTE_CONFIG[voteType];
  window.open(config.url, '_blank');
  recordVoteAttempt(voteType);
  showToast(`✔ ${config.name} dibuka — terima kasih vote!`);
}

function recordVoteAttempt(voteType){
  const voteData = getVoteData();
  voteData[voteType] = Date.now();
...
function isVoteCooldownActive(voteType){
  const voteData = getVoteData();
  if(!voteData[voteType]) return false;
  const lastVoteTime = voteData[voteType];
  const cooldownMs = VOTE_COOLDOWN_HOURS * 60 * 60 * 1000;
  return Date.now() - lastVoteTime < cooldownMs;
}

function getRemainingCooldownTime(voteType){
  const voteData = getVoteData();
  if(!voteData[voteType]) return null;
  const lastVoteTime = voteData[voteType];
  const cooldownMs = VOTE_COOLDOWN_HOURS * 60 * 60 * 1000;
  const remainingMs = cooldownMs - (Date.now() - lastVoteTime);
...
function updateVoteUI(){
  Object.keys(VOTE_CONFIG).forEach(voteType => {
    const btn = document.getElementById(`vote-btn-${voteType}`);
    const cooldownEl = document.getElementById(`vote-cooldown-${voteType}`);
    if(!btn) return;
    const isOnCooldown = isVoteCooldownActive(voteType);
    const remainingTime = getRemainingCooldownTime(voteType);
    if(isOnCooldown && remainingTime){
      btn.disabled = true;
      btn.style.opacity = '0.5';
      if(cooldownEl){
        cooldownEl.textContent = `⏳ Bisa vote lagi dalam ${remainingTime}`;
